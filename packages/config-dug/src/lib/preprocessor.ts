...
const getPreprocessorByZodType = (schema: z.ZodFirstPartySchemaTypes): ((arg: string | undefined) => unknown) => {
...
const getSchemaWithPreprocessor = (schema: z.ZodTypeAny): z.ZodEffects<z.ZodTypeAny, any, unknown> => {
  return z.preprocess(getPreprocessorByZodType(schema) as (arg: unknown) => unknown, schema);
};

export { getSchemaWithPreprocessor };
